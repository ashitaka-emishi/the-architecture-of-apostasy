# The Grammar of Apostasy

This folder is the internal grammar for the larger Architecture of Apostasy archive.
It keeps the shared vocabulary, ontology, chapter pattern, and analysis questions aligned
as essays and case studies are developed from different angles.

Use this material as the project canon for terms and structure, not as a finished public
essay. The public-facing works can vary in voice, but they should remain legible inside
this grammar.

## Files

- [Constitution](constitution.md)
- [Canonical Ontology](canonical-ontology.md)
- [Editorial Style Guide](editorial-style-guide.md)
- [Agent Contracts](agent-contracts.md)
- [Human Review Rubric](human-review-rubric.md)
- [Christological Resolution Examples](christological-resolution-examples.md)
- [Aru Vaen And The Grammar](aru-vaen-and-the-grammar.md)
- [Prophetic Warning Vs Omniscient Verdict](prophetic-warning-vs-verdict.md)
- [Faithful Institutions In The Grammar](faithful-institutions.md)
- [Conversation Summary](conversation-summary.md)

## Operating Artifacts

- `checklists/` contains human theological validation rubrics.
- `../../ai-system/` contains prompts, schemas, scripts, machine-readable graphs, and other AI-facing implementation files.
- `../aspects/` contains theological sub-aspects and case-study scaffolds.

## Core Ontology

God
→ Logos
→ Creation
→ Principalities (created to govern)
→ Fall of the Powers
→ Disordered Logos (false symbolic order)
→ Fallen Liturgies (embodied rituals)
→ Formation of the Chemical Temple
→ Individuals / Institutions / Civilizations
→ Christ
→ Restoration

## Universal Analysis Questions

1. What created good is being examined?
2. Which principality governs this domain?
3. How has that principality fallen?
4. What Disordered Logos is proclaimed?
5. What liturgies embody it?
6. How is the Chemical Temple formed?
7. What sacrificial system emerges?
8. How does Christ restore it?

## Suggested Series

- The Logos and the Powers
- War as a Fallen Liturgy
- Politics as a Fallen Liturgy
- The Chemical Temple
- Addiction and the Disordered Logos
- Marriage as Holy Liturgy
- The Church as the True Liturgy
- Technology as Liturgy
- Consumerism as Worship
- Entertainment as Formation
- Education and Symbolic Formation
- Money and Mammon
- Death, Sacrifice, and Resurrection

## Analysis Pipeline

Biblical → Historical Theology → Philosophy → Psychology → Anthropology → Systems Mapping → Diagramming → Critique → Editorial → Canonical Integration.
